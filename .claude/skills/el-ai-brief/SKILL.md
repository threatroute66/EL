...
name: el-ai-brief
description: |
  Fulfil an EL executive-brief request that the `el` CLI emitted via
  the Claude Code path. Use this whenever a case directory under
  `/opt/EL/cases/` contains a `reports/_ai_brief_request.json` file,
  OR when the user invokes `/el-ai-brief` directly (with no arg =
  process every case that has a pending request; with an arg =
  process only that case-id). EL auto-writes the request file when
  it detects it's running inside a Claude Code session (CLAUDECODE=1)
  or when the operator passed `--defer-ai-brief` outside Claude
  Code. Either way the brief is generated by Claude — this skill
  just transports it to disk and re-renders the executive HTML+PDF
  so the six-section brief lands in the final report.
---
