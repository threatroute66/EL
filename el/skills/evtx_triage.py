...
def iter_events(csv_path: Path) -> list[EvtxEvent]:
    """Parse the EvtxECmd CSV into EvtxEvent objects. Non-integer EventIds
    and rows missing required columns are silently skipped — real exports
    have the occasional malformed row (e.g. channel column containing
    Level when a field has a newline)."""
...
def detect_scheduled_task_creation(
    ...
    ts106 = [e for e in events
             if "taskscheduler" in e.channel.lower() and e.event_id == 106]
    hits = s4698 + ts106
    if hits:
        first, last = _summary(hits)
...
def detect_wmi_persistence(
    ...
    hits = []
    for e in events:
        if "wmi-activity" not in e.channel.lower():
            continue
        if e.event_id in (5860, 5861):
            hits.append(e)
    if hits:
        first, last = _summary(hits)
        out.append(LMHit(
    ...
    suspicious_5857 = []
    for e in events:
        if "wmi-activity" not in e.channel.lower() or e.event_id != 5857:
            continue
        payload = " ".join(e.payload.values()).lower()
...
def detect_powershell_remoting_inbound(
    ...
    out: list[LMHit] = []
    winrm91 = [e for e in events
               if "winrm" in e.channel.lower() and e.event_id == 91]
    ps4104 = [e for e in events
              if "powershell/operational" in e.channel.lower()
              and e.event_id == 4104]
...
def detect_rdp_destination(
    ...
    out: list[LMHit] = []
    ts1149 = [e for e in events
              if "remoteconnectionmanager" in e.channel.lower()
              and e.event_id == 1149]
