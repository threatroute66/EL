/*
  HIDDEN COBRA (DPRK / Lazarus) — US-CERT YARA rule pack, vendored verbatim.

  Source : Yara-Rules/rules  malware/APT_HiddenCobra.yar
           https://raw.githubusercontent.com/Yara-Rules/rules/refs/heads/master/malware/APT_HiddenCobra.yar
  Authors: US-CERT / NCCIC Code Analysis Team (rule meta carries per-rule
           report URLs + MD5s).
  Reports: TA17-164A (DDoS botnet / DeltaCharlie), TA17-318A (FALLCHILL RAT),
           MAR-10132963, MAR-10135536-B/-G/.11.

  FALLCHILL relevance: the MAR-10135536 series is the FALLCHILL / HIDDEN COBRA
  proxy-RAT reporting; the rules `Unauthorized_Proxy_Server_RAT` (Incident
  10135536 / MAR-10135536-B) and `NK_SSL_PROXY` (MAR-10135536-G) are the
  FALLCHILL-era proxy/SSL-tunnel detections. The pack also covers the DDoS
  botnet binaries and the r4 wipers.

  DO NOT edit the byte signatures — they are authoritative US-CERT output.
  EL appends this file to the per-case rule set (behind the .yar self-match
  guard in yara_hunt.scan_paths). See el/intel/malware_families.py for the
  string-level HIDDEN COBRA fingerprint used on the dump-strings path.
*/
