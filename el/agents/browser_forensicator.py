...
class BrowserForensicatorAgent(Agent):
    ...
    def run(self, ctx: AgentContext) -> list[Finding]:
        ...
        # Chromium web-storage LevelDB (Local Storage / IndexedDB) — runs
        # even when Hindsight is unavailable; Snappy-aware, dependency-free.
        ldb_findings = self._run_leveldb(ctx, [root])
        out.extend(ldb_findings)

        if not places and not chromium_profiles and not ldb_findings:
            return [self.emit(ctx, Finding(
                case_id=ctx.case_id, agent=self.name, confidence="insufficient",
                claim=(f"No Firefox places.sqlite, Chromium profile dirs "
                       f"(History+Cookies), or LevelDB web-storage found "
                       f"under {root}"),
            ))]
    ...
    def _run_leveldb(self, ctx: AgentContext,
        ...
        """Parse Chromium LevelDB web-storage (Local Storage / IndexedDB /
        Session Storage) under *scan_dirs* and emit a grounded inventory.
        Snappy-aware and dependency-free, so it works where Hindsight can't.
        No-op when no store yields records."""
