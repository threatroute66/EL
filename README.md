...
| [NPS M57-Jean](https://digitalcorpora.org/corpora/scenarios/m57-jean/) | NTFS E01 multi-part (Windows XP) | 3 GB | **EL arrived at the canonical answer neither [Basilmellow](https://github.com/Basilmellow/Autopsy-M57-Linux-Forensics) nor [jynxora](https://github.com/jynxora/M57-Jean-Case-Analysis) reached**: BEC / pretexting exfil (H_BEC_ACCOUNT_TAKEOVER score 30 leading, gap +17 over insider-exfil). Email forensicator pulled the actual exfil email from `Jean--outlook.pst Sent Items`: `jean@m57.biz → tuckgorge@gmail.com` subject `"RE: Please send me the information now"` with `1_m57biz.xls (291840B)` attached, DISPLAYED as going to `alison@m57.biz` — classic display-name/SMTP-mismatch impersonation signature. Executive Narrative renders the full chain with `[finding_id]` citations end-to-end in 16 min |
