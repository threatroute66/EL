...
| ~~CyLR output~~ ✅ | Shipped triage zip-detect (`CyLR_Collection_Log_<TS>.log` marker at root OR ≥5 Linux FS-root entries under var/log / etc / home / root). LinuxForensicatorAgent auto-extracts into `<case>/raw/cylr/` then runs every existing Linux detector against the extracted tree — auditd / utmp / systemd_journal / webserver_access / bash_history / cron / cracker_tooling / rootkit scanners — without per-tool wiring (CyLR's extracted layout IS a Linux FS root). Validated end-to-end on operator's 24 MB `rubicon.zip`: triage routed correctly → cracker_tooling matched rockyou.txt reference (T1110) → wtmp + systemd_journal + sudo escalation all surfaced → leading hypothesis H_APT_ESPIONAGE (score=4). Commit `88526d6`. | Operator-built CyLR collection at `/mnt/hgfs/hackathon/rubicon.zip`; CyLR GitHub release labs |
...
| ~~CyLR triage + auto-extract~~ ✅ | `88526d6` | Triage zip-detect (canonical `CyLR_Collection_Log_<TS>.log` marker OR ≥5 Linux FS-root entries). LinuxForensicatorAgent auto-extracts to `<case>/raw/cylr/` then runs every existing Linux detector against the extracted tree (no per-tool wiring — CyLR's layout IS a Linux FS root). Validated on operator's 24 MB `rubicon.zip` → cracker_tooling caught rockyou.txt (T1110) → systemd_journal parsed 126,988 entries + 87 sudo escalations → H_APT_ESPIONAGE (score=4). |
