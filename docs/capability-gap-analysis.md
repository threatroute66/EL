...
| ~~Truncated linear captures (DTB above captured range)~~ ◑ | Handled gracefully, not fully recovered. DumpIt/Comae 4 GB linear dumps on VMs that remap RAM above the 4 GB MMIO hole omit the page tables, so vol3 builds no kernel layer (`DtbSelfRef64bit … > 0xffffffff`). `el.skills.vol3.scan_windows_banner` (raw ntoskrnl-banner scan, no symbols) confirms the OS + exact build, triage emits a precise `insufficient`, and the image routes to string/IOC carve. Structured process/network plugins stay unavailable (same limit the original team's Volatility 2.6 hit). Validated on the 3-host 2019 Narcos memory set (`narcos-mem-r2`): builds 1809/1803/1709 all identified; carve recovered TrueCrypt password + Quasar C2 + Protonmail accounts. | 2019 Narcos (digitalcorpora) memory dumps |
...
| ~~Zeek `conn.log` / `http.log` etc. (batch ingest)~~ ✅ | Shipped `el.skills.zeek_json` — ingests EXISTING Zeek JSON output (conn/dns/http/ssl/x509/files/dhcp…), complementing `el.skills.zeek` (which only *generates* logs from a pcap). `ts`→UTC, per-protocol accessors, `find()`. Fanned out per host by `LogCorpusAgent`; validated on the Northstar SOC corpus (10,887 records / 9 log types). | Zeek-published training data, LANL-netflow |
| ~~SOC multi-host log corpus~~ ✅ | Shipped `LogCorpusAgent` + triage `log-corpus` kind: auto-detects a per-host log directory and fans each source to a native parser — `el.skills.{evtx_xml,ecar,cisco_asa,snort_alert,zeek_json,webserver_access/iis_w3c,syslog_rfc5424}`. Lifts H_PROCESS_INJECTION / H_BRUTE_FORCE / H_SCAN_RECON from high-signal counts and writes per-host attack-stage Events into the Kùzu graph so the `correlator` ties a cross-host chain (recon→web-shell→injection→H_APT_ESPIONAGE; shared attacker IP→H_LATERAL_MOVEMENT). Validated on the Northstar corpus (30 sources / 12 hosts / 52,433 records). | malware-traffic / SOC-class multi-host log sets |
| ~~Windows Event log XML exports~~ ✅ | Shipped `el.skills.evtx_xml` — parses pre-exported `<Events><Event>` XML (which EvtxECmd can't read; it needs binary `.evtx`), streamed via iterparse; EventID/time/computer/provider + EventData pairs; logon/process accessors. | wevtutil / Get-WinEvent exports |
...
|---|---|---|
| Microsoft Teams (LevelDB + IndexedDB) | **parser available, per-app wiring pending** — the LevelDB dep gap is closed: `el.skills.chromium_leveldb` (pure-Python Snappy, zstd-if-present; recovers superseded + tombstoned records) is shipped and wired into the browser + android agents. Teams/Slack/Discord Electron stores parse with the same primitive; only the per-app container path + semantics remain to wire. | Any Teams-using workstation image |
| Slack (same Electron LevelDB shape) | **parser available, per-app wiring pending** — same `chromium_leveldb` primitive | Slack desktop on any dev workstation |
...
| Discord IndexedDB | **parser available, per-app wiring pending** — `chromium_leveldb` reads the IndexedDB leveldb; per-app semantics pending | Consumer dev workstation |
